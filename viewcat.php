...
// $Id: viewcat.php,v 1.12 2005/03/27 12:11:07 w4z004 Exp $
// ------------------------------------------------------------------------ //
...
$cid = (int)$_GET['cid'];
$GLOBALS['xoopsOption']['template_main'] = 'xdir_viewcat.html';
...
if (count($arr) > 0) {
    ...
    foreach ($arr as $ele) {
        ...
        foreach ($sub_arr as $sub_ele) {
            ...
            $infercategories .= '<a href="' . XOOPS_URL . '/modules/hiperlinks/viewcat.php?cid=' . $sub_ele['cid'] . '">' . $chtitle . '</a>';
...
if ($numrows > 0) {
    ...
    while (list($lid, $cid, $ltitle, $address, $address2, $city, $state, $zip, $country, $phone, $fax, $email, $url, $logourl, $status, $time, $hits, $rating, $votes, $comments, $premium, $description) = $xoopsDB->fetchRow($result)) {
        ...
        $path = str_replace('/', " <img src='" . XOOPS_URL . "/modules/hiperlinks/images/arrow.gif' board='0' alt=''> ", $path);
