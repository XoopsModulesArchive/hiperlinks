...
// $Id: index.php,v 1.12 2005/03/27 12:11:06 w4z004 Exp $
// ------------------------------------------------------------------------ //
...
$GLOBALS['xoopsOption']['template_main'] = 'xdir_index.html';
require XOOPS_ROOT_PATH . '/header.php';
$result = $xoopsDB->query('SELECT cid, title, imgurl FROM ' . $xoopsDB->prefix('xdir_cat') . ' WHERE pid = 0 ORDER BY title') || exit('Error');
$count = 1;
while ($myrow = $xoopsDB->fetchArray($result)) {
    ...
    foreach ($arr as $ele) {
        ...
        $subcategories .= '<a href="' . XOOPS_URL . '/modules/hiperlinks/viewcat.php?cid=' . $ele['cid'] . '">' . $chtitle . '</a>';
...
[$numrows] = $xoopsDB->fetchRow($xoopsDB->query('select count(*) from ' . $xoopsDB->prefix('xdir_links') . ' where status>0'));
...
while (list($lid, $cid, $ltitle, $address, $address2, $city, $state, $zip, $country, $phone, $fax, $email, $url, $logourl, $status, $time, $hits, $rating, $votes, $comments, $premium, $description) = $xoopsDB->fetchRow($result)) {
    if ($isadmin) {
        $adminlink = '<a href="' . XOOPS_URL . '/modules/hiperlinks/admin/?op=modLink&lid=' . $lid . '"><img src="' . XOOPS_URL . '/modules/hiperlinks/images/editicon.gif" border="0" alt="' . _MD_EDITTHISLINK . '"></a>';
    } else {
        $adminlink = '';
    }
